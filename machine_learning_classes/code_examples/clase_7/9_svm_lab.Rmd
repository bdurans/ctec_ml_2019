...
# prediction <- predict(svmfit.flex, dat[train, ], decision.values = T)
obj <- roc.curve(scores.class0 = fitted, weights.class0=as.numeric(dat[-train, "y"]),
                       curve=TRUE)
plot(obj)
...
matriz_confusion <- table(true = dat[-train, "y"], 
      pred = predict(tune.out$best.model, newx = dat[-train, ]))
```


Ahora obtenemos la exactitud

\begin{equation}Exactitud=\frac{VP+VN}{Total}\end{equation}

- *Exactitud Total* (observaciones clasificadas apropiadamente) = 60% (54 verdadedors positivos y verdaderos negativos).

```{r}
print((matriz_confusion[1,1] + matriz_confusion[2,2]) / sum(matriz_confusion) )
```

- *Tasa de error*

```{r}
print((matriz_confusion[1,2] + matriz_confusion[2,1]) / sum(matriz_confusion) )
```
